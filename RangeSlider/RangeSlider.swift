...
class RangeSliderThumbLayer: CALayer {
	...
	override func draw(in ctx: CGContext) {
		...
		let thumbFrame = bounds.insetBy(dx: 1.0, dy: 1.0) //bounds.insetBy(dx: 2.0, dy: 2.0)
...
public class RangeSlider: UIControl {
	...
	func getThumbLayerCloserToTouch(touchLocation: CGPoint) -> RangeSliderThumbLayer {
		let xLower = lowerThumbLayer.frame.origin.x
		let yLower = lowerThumbLayer.frame.origin.y
		let xUpper = upperThumbLayer.frame.origin.x
		let yUpper = upperThumbLayer.frame.origin.y
		
		let distanceSquaredToLower = pow(xLower - touchLocation.x, 2.0) + pow(yLower - touchLocation.y, 2.0)
		let distanceSquaredToUpper = pow(xUpper - touchLocation.x, 2.0) + pow(yUpper - touchLocation.y, 2.0)
		
		if distanceSquaredToLower < distanceSquaredToUpper {
			return lowerThumbLayer
		} else  {
			return upperThumbLayer
		}
	}
	...
	override public func beginTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
		...
		// Alternative method (Calculating squared distance to each thumb):
		let closerThumb = getThumbLayerCloserToTouch(touchLocation: previouslocation)
		
		if closerThumb == lowerThumbLayer {
			let lowerThumbCenter = CGFloat(positionForValue(lowerValue))
			let deltaLocation = Double(previouslocation.x - lowerThumbCenter)
			let deltaValue = (maximumValue - minimumValue) * deltaLocation / Double(bounds.width - bounds.height)
			lowerValue = boundValue(lowerValue + deltaValue, toLowerValue: minimumValue, upperValue: upperValue - gapBetweenThumbs)
			lowerThumbLayer.highlighted = true
		} else if closerThumb == upperThumbLayer {
			let upperThumbCenter = CGFloat(positionForValue(upperValue))
			let deltaLocation = Double(previouslocation.x - upperThumbCenter)
			let deltaValue = (maximumValue - minimumValue) * deltaLocation / Double(bounds.width - bounds.height)
	...
	override public func continueTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
		...
		if lowerThumbLayer.highlighted && lowerValue + deltaValue > upperValue - gapBetweenThumbs {
			lowerThumbLayer.highlighted = false
			upperThumbLayer.highlighted = true
		} else if upperThumbLayer.highlighted && upperValue + deltaValue < lowerValue + gapBetweenThumbs {
